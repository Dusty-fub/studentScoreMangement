	...
	public List<Exam> getExamList(String sql, List<Object> param) {
		...
		try {
			...
			while (rs.next()) {
				...
				// 查询班级
				Clazz clazz = (Clazz) getObject(Clazz.class, "SELECT * FROM clazz WHERE id=?",
						new Object[] { exam.getClazzid() });
				// 查询年级
				Grade grade = (Grade) getObject(Grade.class, "SELECT * FROM grade WHERE id=?",
						new Object[] { exam.getGradeid() });
				// 查询科目
				Course course = (Course) getObject(Course.class, "SELECT * FROM course WHERE id=?",
						new Object[] { exam.getCourseid() });
				// 添加
				exam.setClazz(clazz);
				exam.setGrade(grade);
