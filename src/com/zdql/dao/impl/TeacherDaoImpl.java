		...
		try {
			...
			while (rs.next()) {
				...
				for (Object obj : objList) {
					...
					// 查询班级
					Clazz sclazz = (Clazz) getObject(Clazz.class, "SELECT * FROM clazz WHERE id=?",
							new Object[] { item.getClazzid() });
					Grade sgrade = (Grade) getObject(Grade.class, "SELECT * FROM grade WHERE id=?",
							new Object[] { item.getGradeid() });
					Course course = (Course) getObject(Course.class, "SELECT * FROM course WHERE id=?",
							new Object[] { item.getCourseid() });
