...
import com.zdql.bean.User;
import com.zdql.dao.impl.BaseDaoImpl;
import com.zdql.dao.impl.ExamDaoImpl;
import com.zdql.dao.impl.ScoreDaoImpl;
import com.zdql.dao.impl.StudentDaoImpl;
import com.zdql.dao.impl.TeacherDaoImpl;
import com.zdql.dao.inter.BaseDaoInter;
import com.zdql.dao.inter.ExamDaoInter;
import com.zdql.dao.inter.ScoreDaoInter;
import com.zdql.dao.inter.StudentDaoInter;
import com.zdql.dao.inter.TeacherDaoInter;
import com.zdql.tools.ExcelTool;
...
public class ScoreService {
	...
	public void addExam(Exam exam) throws Exception {
		...
		try {
			...
			int examid = dao.insertReturnKeysTransaction(conn,
					...
					new Object[] { exam.getName(), exam.getTime(), exam.getRemark(), exam.getType(), exam.getGradeid(),
							exam.getClazzid() });
			...
			if (exam.getType() == Exam.EXAM_GRADE_TYPE) { // 年级统考
				...
				List<Object> couObjList = dao.getList(Course.class,
						"SELECT courseid id FROM grade_course WHERE gradeid=?", new Object[] { exam.getGradeid() });

				// 查询该年级下的学生
				List<Object> stuList = dao.getList(Student.class, "SELECT id, clazzid FROM student WHERE gradeid=?",
				...
				// 查询该班级下的学生
				List<Object> stuList = dao.getList(Student.class, "SELECT id FROM student WHERE clazzid=?",
	...
	private List<Object> getColumn(Exam exam) {
		...
		if (exam.getType() == Exam.EXAM_GRADE_TYPE) { // 年级考试
			...
			// 获取某科
			list = dao.getList(Course.class, "SELECT * FROM course WHERE id=?", new Object[] { exam.getCourseid() });
	...
	public void exportScore(HttpServletResponse response, Exam exam) {
		...
		List<Map<String, Object>> list = dao.getScoreList(exam);
		// 获取考试信息
		Exam em = (Exam) dao.getObject(Exam.class, "SELECT name, time FROM exam WHERE id=?",
				new Object[] { exam.getId() });
		// 设置文件名
